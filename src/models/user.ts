...
export default class User {
    public static getByUsername(name: string): Promise<User | null> {
    ...
    // 用户名密码认证
    public static authenticate(userInfo: ParamsUserInfo): Promise<number> {
        return new Promise(async (resolve, reject) => {
            try {
    ...
    public static issueToken(id: number) {
        return new Promise((resolve, reject) => {
            redis.hgetall('user:' + id, (err, userInfo) => {
                if (err) {
                    reject(err);
    ...
    private static getId(name: string): Promise<number | null> {
        return new Promise((resolve, reject) => {
    ...
    public constructor(userInfo: ParamsUserInfo) {
